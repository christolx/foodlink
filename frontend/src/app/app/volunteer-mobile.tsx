...
import { getOrCreateConversation, type Donation, type Pickup, type Profile } from "@/lib/api";
  ...
  return (
    ...
    >
      ...
      <VolunteerMobileProposalCard
        ...
        onCreate={onCreate}
        onChatDonor={selectedDonation?.donorId ? () => void startChat(selectedDonation.donorId) : undefined}
        onChatReceiver={selectedReceiver?.userId ? () => void startChat(selectedReceiver.userId) : undefined}
      />
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            className="inline-flex items-center gap-1.5 rounded-full bg-[#e5f1df] px-3 py-1 text-xs font-black text-[#14733a]"
            onClick={() => { setChatConvId(null); setChatOpen(true); }}
          >
            <AppIcon name="message" className="h-3.5 w-3.5" />
            Open
            {unread > 0 && (
              <span className="ml-0.5 rounded-full bg-[#ffbd1a] px-1.5 text-[0.6rem] text-[#10140d]">
                {unread}
              </span>
            )}
          </button>
      ...
      {chatOpen && (
        <ChatPanel
          token={token}
          myUserId={myUserId}
          initialConvId={chatConvId ?? undefined}
          onClose={() => { setChatOpen(false); setChatConvId(null); }}
        />
      )}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-3">
        <article className="grid grid-cols-[3.5rem_1fr_auto] items-center gap-3 rounded-xl border border-[#e4ddcf] bg-[#fbfaf3] p-3">
          <DonationThumbnail donation={donation} size="sm" />
          <div className="min-w-0">
          ...
          {onChatDonor ? (
            <button
              type="button"
              aria-label="Chat donor"
              className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#e5f1df] text-[#14733a]"
              onClick={onChatDonor}
            >
        ...
        <article className="grid grid-cols-[3.5rem_1fr_auto] items-center gap-3 rounded-xl border border-[#e4ddcf] bg-[#fbfaf3] p-3">
          ...
          {onChatReceiver ? (
            <button
              type="button"
              aria-label="Chat receiver"
              className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#e5f1df] text-[#14733a]"
              onClick={onChatReceiver}
            >
              <AppIcon name="message" className="h-4 w-4" />
            </button>
          ) : <span />}
