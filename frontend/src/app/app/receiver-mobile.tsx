...
function ReceiverPriorityProposalCard({
  ...
  async function openChat() {
    if (!proposal?.volunteerId) return;
            ...
            <div className="grid gap-2 text-sm font-bold text-[#1f2a23]">
              ...
              <span className="flex items-center justify-between gap-2">
                ...
                {proposal.volunteerId && (
                  <button
                    type="button"
                    aria-label="Chat volunteer"
                    className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#e5f1df] text-[#14733a]"
...
function ReceiverMobileProposalDetails({
  ...
  async function openChat() {
    const otherId = proposal.volunteerId;
    if (!otherId) return;
    const conv = await getOrCreateConversation(token, otherId);
    setChatVolunteerId(conv.id);
  }
    ...
    <SlidePanel title="Proposal details" icon="package" onClose={onClose}>
      <div className="grid gap-4">
        ...
        <ReceiverDetailNote
          ...
          secondaryAction="Chat on FoodLink"
          onSecondaryActionClick={() => void openChat()}
        />
      </div>
    </SlidePanel>
    {chatVolunteerId && (
      <ChatPanel
        token={token}
        myUserId={myUserId}
        initialConvId={chatVolunteerId}
        onClose={() => setChatVolunteerId(null)}
      />
    )}
    </>
  );
